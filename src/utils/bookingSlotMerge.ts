/**
 * Merge consecutive 30-minute (or any) slot segments into continuous ranges
...
/** Per-station slot lists; supports legacy `slots` (same for every station). */
...
/** Insert rows for Razorpay / pending_payment flows: one row per contiguous block per station. */
